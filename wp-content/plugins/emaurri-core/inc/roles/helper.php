...
if ( ! function_exists( 'emaurri_core_include_role_custom_fields' ) ) {
	/**
	 * Function that includes role custom fields files
	 */
	function emaurri_core_include_role_custom_fields() {
		foreach ( glob( EMAURRI_CORE_INC_PATH . '/roles/*/role-fields.php' ) as $role_fields ) {
			include_once $role_fields;
		}
	}

	add_action( 'qode_framework_action_custom_user_fields', 'emaurri_core_include_role_custom_fields' );
}

if ( ! function_exists( 'emaurri_core_register_role_custom_fields' ) ) {
	/**
	 * Function that registers role custom fields files
	 */
	function emaurri_core_register_role_custom_fields() {
		do_action( 'emaurri_core_action_register_role_custom_fields' );
	}

	add_action( 'qode_framework_action_custom_user_fields', 'emaurri_core_register_role_custom_fields', 11 );
}

if ( ! function_exists( 'emaurri_core_profile_get_social_params' ) ) {
	/**
	 * Returns owner params
	 */
	function emaurri_core_profile_get_social_params( $params ) {
		...
		if ( $enable ) {
			...
			$params['behance']   = get_user_meta( $user_id, 'qodef_user_behance', true );
	...
	add_filter( 'emaurri_membership_filter_user_params', 'emaurri_core_profile_get_social_params' );
}

if ( ! function_exists( 'emaurri_core_social_edit_profile_fields' ) ) {
	function emaurri_core_social_edit_profile_fields( $page, $params ) {
		$roles_social_scope = apply_filters( 'emaurri_core_filter_role_social_array', array( 'administrator', 'author' ) );
		$enable             = false;
	...
	add_action( 'emaurri_membership_action_after_dashboard_edit_profile_fields', 'emaurri_core_social_edit_profile_fields', 15, 2 );
}

if ( ! function_exists( 'emaurri_core_user_profile_social_update' ) ) {
	function emaurri_core_user_profile_social_update( $options, $user_id ) {
		...
		if ( $enable ) {
			...
			update_user_meta( $user_id, 'qodef_user_behance', $options['qodef_user_behance'] );
	...
	add_action( 'emaurri_membership_action_update_user_profile', 'emaurri_core_user_profile_social_update', 10, 2 );
}
