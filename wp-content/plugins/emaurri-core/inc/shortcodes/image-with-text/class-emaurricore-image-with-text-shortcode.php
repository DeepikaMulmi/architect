...
if ( ! function_exists( 'emaurri_core_add_image_with_text_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function emaurri_core_add_image_with_text_shortcode( $shortcodes ) {
		$shortcodes[] = 'EmaurriCore_Image_With_Text_Shortcode';

		return $shortcodes;
	}

	add_filter( 'emaurri_core_filter_register_shortcodes', 'emaurri_core_add_image_with_text_shortcode' );
}

if ( class_exists( 'EmaurriCore_Shortcode' ) ) {
	class EmaurriCore_Image_With_Text_Shortcode extends EmaurriCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'emaurri_core_filter_image_with_text_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'emaurri_core_filter_image_with_text_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( EMAURRI_CORE_SHORTCODES_URL_PATH . '/image-with-text' );
			$this->set_base( 'emaurri_core_image_with_text' );
			$this->set_name( esc_html__( 'Image With Text', 'emaurri-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image with text element', 'emaurri-core' ) );
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'emaurri_core_image_with_text', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return emaurri_core_get_template_part( 'shortcodes/image-with-text', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
