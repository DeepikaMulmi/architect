...
if ( ! function_exists( 'emaurri_core_add_custom_font_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function emaurri_core_add_custom_font_shortcode( $shortcodes ) {
		$shortcodes[] = 'EmaurriCore_Custom_Font_Shortcode';

		return $shortcodes;
	}

	add_filter( 'emaurri_core_filter_register_shortcodes', 'emaurri_core_add_custom_font_shortcode' );
}

if ( class_exists( 'EmaurriCore_Shortcode' ) ) {
	class EmaurriCore_Custom_Font_Shortcode extends EmaurriCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'emaurri_core_filter_custom_font_layouts', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( EMAURRI_CORE_SHORTCODES_URL_PATH . '/custom-font' );
			$this->set_base( 'emaurri_core_custom_font' );
			$this->set_name( esc_html__( 'Custom Font', 'emaurri-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays custom font with provided parameters', 'emaurri-core' ) );
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'title',
					'title'      => esc_html__( 'Title Text', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'name'        => 'font_size_1366',
					'title'       => esc_html__( 'Font Size', 'emaurri-core' ),
					'description' => esc_html__( 'Set responsive style value for screen size 1366', 'emaurri-core' ),
					'group'       => esc_html__( 'Screen Size 1366', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'name'        => 'font_size_1024',
					'title'       => esc_html__( 'Font Size', 'emaurri-core' ),
					'description' => esc_html__( 'Set responsive style value for screen size 1024', 'emaurri-core' ),
					'group'       => esc_html__( 'Screen Size 1024', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'name'        => 'font_size_768',
					'title'       => esc_html__( 'Font Size', 'emaurri-core' ),
					'description' => esc_html__( 'Set responsive style value for screen size 768', 'emaurri-core' ),
					'group'       => esc_html__( 'Screen Size 768', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'name'        => 'font_size_680',
					'title'       => esc_html__( 'Font Size', 'emaurri-core' ),
					'description' => esc_html__( 'Set responsive style value for screen size 680', 'emaurri-core' ),
					'group'       => esc_html__( 'Screen Size 680', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public function render( $options, $content = null ) {
			...
			return emaurri_core_get_template_part( 'shortcodes/custom-font', 'variations/' . $atts['layout'] . '/templates/custom-font', '', $atts );
		}
		...
		private function set_responsive_styles( $atts ) {
			...
			foreach ( $screen_sizes as $screen_size ) {
				...
				if ( ! empty( $styles ) ) {
					add_filter( 'emaurri_core_filter_add_responsive_' . $screen_size . '_inline_style_in_footer', function ( $style ) use ( $unique_class, $styles ) {
