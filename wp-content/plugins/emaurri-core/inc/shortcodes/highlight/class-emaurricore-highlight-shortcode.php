...
if ( ! function_exists( 'emaurri_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function emaurri_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'EmaurriCore_Highlight_Shortcode';

		return $shortcodes;
	}

	add_filter( 'emaurri_core_filter_register_shortcodes', 'emaurri_core_add_highlight_shortcode' );
}

if ( class_exists( 'EmaurriCore_Shortcode' ) ) {
	class EmaurriCore_Highlight_Shortcode extends EmaurriCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( EMAURRI_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'emaurri_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'emaurri-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'emaurri-core' ) );
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Disable Text Line Break', 'emaurri-core' ),
					'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'emaurri-core' ),
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Highlight Text Position', 'emaurri-core' ),
					'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Color', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Background Color', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public function render( $options, $content = null ) {
			...
			return emaurri_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
