...
if ( ! function_exists( 'emaurri_core_add_single_image_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function emaurri_core_add_single_image_shortcode( $shortcodes ) {
		$shortcodes[] = 'EmaurriCore_Single_Image_Shortcode';

		return $shortcodes;
	}

	add_filter( 'emaurri_core_filter_register_shortcodes', 'emaurri_core_add_single_image_shortcode' );
}

if ( class_exists( 'EmaurriCore_Shortcode' ) ) {
	class EmaurriCore_Single_Image_Shortcode extends EmaurriCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'emaurri_core_filter_single_image_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'emaurri_core_filter_single_image_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( EMAURRI_CORE_SHORTCODES_URL_PATH . '/single-image' );
			$this->set_base( 'emaurri_core_single_image' );
			$this->set_name( esc_html__( 'Single Image', 'emaurri-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image element', 'emaurri-core' ) );
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'emaurri_core_single_image', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return emaurri_core_get_template_part( 'shortcodes/single-image', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
