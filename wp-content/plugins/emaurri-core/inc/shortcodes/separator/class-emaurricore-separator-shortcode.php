...
if ( ! function_exists( 'emaurri_core_add_separator_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function emaurri_core_add_separator_shortcode( $shortcodes ) {
		$shortcodes[] = 'EmaurriCore_Separator_Shortcode';

		return $shortcodes;
	}

	add_filter( 'emaurri_core_filter_register_shortcodes', 'emaurri_core_add_separator_shortcode', 9 );
}

if ( class_exists( 'EmaurriCore_Shortcode' ) ) {
	class EmaurriCore_Separator_Shortcode extends EmaurriCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( EMAURRI_CORE_SHORTCODES_URL_PATH . '/separator' );
			$this->set_base( 'emaurri_core_separator' );
			$this->set_name( esc_html__( 'Separator', 'emaurri-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays separator with provided parameters', 'emaurri-core' ) );
			...
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'position',
					'title'      => esc_html__( 'Position', 'emaurri-core' ),
					'options'    => array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'width',
					'title'      => esc_html__( 'Width (px or %)', 'emaurri-core' ),
					'group'      => esc_html__( 'Style', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'thickness',
					'title'      => esc_html__( 'Thickness (px)', 'emaurri-core' ),
					'group'      => esc_html__( 'Style', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'margin_top',
					'title'      => esc_html__( 'Margin Top (px or %)', 'emaurri-core' ),
					'group'      => esc_html__( 'Style', 'emaurri-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'margin_bottom',
					'title'      => esc_html__( 'Margin Bottom (px or %)', 'emaurri-core' ),
					'group'      => esc_html__( 'Style', 'emaurri-core' ),
				)
			);
		}
		...
		public function render( $options, $content = null ) {
			...
			return emaurri_core_get_template_part( 'shortcodes/separator', 'templates/separator', '', $atts );
		}
