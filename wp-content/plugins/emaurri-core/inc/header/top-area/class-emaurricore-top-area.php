...
class EmaurriCore_Top_Area {
	...
	public function __construct() {
		...
		add_action( 'emaurri_action_before_page_header', array( $this, 'load_template' ) );
		...
		add_filter( 'emaurri_filter_add_inline_style', array( $this, 'set_inline_top_area_styles' ) );
		add_filter( 'emaurri_core_filter_top_area_inner_class', array( $this, 'set_grid_class' ) );
	}

	/**
	 * @return EmaurriCore_Top_Area
	 */
	...
	function is_top_area_enabled() {
		$top_area_enabled = 'yes' === emaurri_core_get_post_value_through_levels( 'qodef_top_area_header' );
	...
	public function set_top_area_height() {
		if ( $this->is_top_area_enabled ) {
			$height         = emaurri_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
	...
	public function is_top_area_transparent() {
		$background_color = emaurri_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
	...
	public function load_template() {
		...
		emaurri_core_template_part( 'header/top-area/', 'templates/top-area', '', $parameters );
	}
	...
	public function set_inline_top_area_styles( $style ) {
		...
		$height       = emaurri_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
		$side_padding = emaurri_core_get_post_value_through_levels( 'qodef_top_area_header_side_padding' );
	...
	public function set_grid_class( $class ) {
		$class .= 'yes' === emaurri_core_get_post_value_through_levels( 'qodef_top_area_header_in_grid' ) ? 'qodef-content-grid' : '';

		return $class;
	}
}

EmaurriCore_Top_Area::get_instance();
