...
if ( ! function_exists( 'emaurri_core_get_header_logo_image' ) ) {
	/**
	 * This function print header logo image
	 *
	 * @param array $parameters
	 */
	function emaurri_core_get_header_logo_image( $parameters = array() ) {
		...
		foreach ( $available_logos as $logo ) {
			...
			$logo_image_id = emaurri_core_get_post_value_through_levels( 'qodef_logo_' . $logo );

			if ( empty( $logo_image_id ) && ! empty( $header_skin ) ) {
				$logo_image_id = emaurri_core_get_post_value_through_levels( 'qodef_logo_main' );
			}
			...
			if ( ! empty( $logo_image_id ) ) {
				$logo_image_attr = array(
					...
					'itemprop' => 'image',
					'alt'      => sprintf( esc_attr__( 'logo %s', 'emaurri-core' ), $logo ),
				);
		...
		if ( ! empty( $customizer_logo ) ) {
			...
			echo apply_filters( 'emaurri_core_filter_get_header_logo_image', emaurri_core_get_template_part( 'header/templates', 'parts/logo', '', $parameters ), $parameters );
		}
	}
}

if ( ! function_exists( 'emaurri_core_get_header_widget_area' ) ) {
	/**
	 ...
	 */
	function emaurri_core_get_header_widget_area( $header_layout = '', $widget_area = 'one' ) {
		...
		if ( $is_enabled ) {
			$parameters = apply_filters(
				'emaurri_core_filter_header_widget_area',
				array(
					'page_id'             => $page_id,
			...
			emaurri_core_template_part( 'header/templates', 'parts/widgets', '', $parameters );
		}
	}
}
