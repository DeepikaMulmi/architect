...
class EmaurriCore_Titles {
	...
	public function __construct() {
		...
		add_filter( 'emaurri_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );

		// Add title area inline styles
		add_filter( 'emaurri_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}

	/**
	 * @return EmaurriCore_Titles
	 */
	...
	function include_layouts() {
		...
		foreach ( glob( EMAURRI_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( EMAURRI_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = emaurri_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'emaurri_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'emaurri_filter_title_template' : 'emaurri_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = emaurri_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = emaurri_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = emaurri_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$image_params       = emaurri_core_get_page_title_image_params();
	...
	function add_inline_styles( $style ) {
		...
		$height                   = emaurri_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile            = emaurri_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_side_padding      = emaurri_core_get_post_value_through_levels( 'qodef_page_title_side_padding' );
		$title_padding            = apply_filters( 'emaurri_core_filter_title_padding', 0 );
		$title_padding_mobile     = apply_filters( 'emaurri_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = emaurri_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color         = emaurri_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params             = emaurri_core_get_page_title_image_params();
		...
		if ( ! empty( $title_side_padding ) ) {
			if ( qode_framework_string_ends_with_space_units( $title_side_padding, true ) ) {
				$styles['padding-left']  = $title_side_padding . '!important';
				$styles['padding-right'] = $title_side_padding . '!important';
			} else {
				$styles['padding-left']  = intval( $title_side_padding ) . 'px !important';
				$styles['padding-right'] = intval( $title_side_padding ) . 'px !important';
			}
		}
		...
		$title_color = emaurri_core_get_post_value_through_levels( 'qodef_page_title_color' );
...
EmaurriCore_Titles::get_instance();
