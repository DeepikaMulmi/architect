...
if ( ! function_exists( 'emaurri_core_get_google_maps_api_key' ) ) {
	/**
	 ...
	 * @return bool|string
	 */
	function emaurri_core_get_google_maps_api_key( $return_type = 'option' ) {
		$option = emaurri_core_get_option_value( 'admin', 'qodef_maps_api_key' );
...
if ( ! function_exists( 'emaurri_core_set_multiple_map_variables' ) ) {
	/**
	 ...
	 */
	function emaurri_core_set_multiple_map_variables( $query = array() ) {
		$map_variables = array();
		...
		if ( is_array( $query ) && count( $query ) ) {
			...
			if ( ! empty( $items ) ) {
				foreach ( $items as $id => $title ) {
					$map_variables['addresses'][] = emaurri_core_generate_map_params( $id, $query['post_type'] );
				}
			}
		}
...
if ( ! function_exists( 'emaurri_core_get_multiple_map' ) ) {
	/**
	 ...
	 * @return string
	 */
	function emaurri_core_get_multiple_map( $query = array() ) {
		$addresses = emaurri_core_set_multiple_map_variables( $query );
		...
		if ( emaurri_core_get_google_maps_api_key( 'is_enabled' ) ) {
			...
			$html = '<p id="qodef-multiple-map-notice">' . esc_html__( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Emaurri Options', 'emaurri-core' ) . '</p>';
		}

		do_action( 'emaurri_core_action_after_multiple_map' );
...
if ( ! function_exists( 'emaurri_core_marker_info_template' ) ) {
	/**
	 * Template with placeholders for marker info window
	 *
	 * uses underscore templates
	 */
	function emaurri_core_marker_info_template() {
	...
	add_action( 'emaurri_core_action_after_multiple_map', 'emaurri_core_marker_info_template' );
}

if ( ! function_exists( 'emaurri_core_marker_template' ) ) {
	/**
	 * Template with placeholders for marker
	 */
	function emaurri_core_marker_template() {
	...
	add_action( 'emaurri_core_action_after_multiple_map', 'emaurri_core_marker_template' );
}
...
if ( ! function_exists( 'emaurri_core_generate_map_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function emaurri_core_generate_map_params( $item_id, $post_type ) {
		$map_params = array();
		...
		//get address params
		$address_array = apply_filters( 'emaurri_core_filter_address_params', array(), $item_id, $post_type );
