		...
		if ( empty( $exclude_option ) || ! in_array( 'space', $exclude_option, true ) ) {
			$this->set_option(
				array(
					...
					'responsive' => true,
					'selectors'  => array(
						'{{WRAPPER}} .qodef-qi-grid'                                        => 'margin-bottom: -{{SIZE}}{{UNIT}};',
						'{{WRAPPER}} .qodef-qi-grid > .qodef-grid-inner'                    => 'margin: 0 calc(-{{SIZE}}{{UNIT}} / 2);',
						'{{WRAPPER}} .qodef-qi-grid > .qodef-grid-inner > .qodef-grid-item' => 'padding: 0 calc({{SIZE}}{{UNIT}} / 2); margin: 0 0 {{SIZE}}{{UNIT}};',
					),
					'group'      => $group,
				)
			);
