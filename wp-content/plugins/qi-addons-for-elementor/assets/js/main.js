					...
					init: function () {
						...
						if ( options.sliderScroll ) {
							var scrollStart        = false,
								$mainHolder        = $holder.hasClass( 'qodef-shortcode' ) ? $holder : $holder.parents( '.qodef-shortcode' ),
								enableFocus        = $mainHolder.hasClass( 'qodef--focus-in-viewport' ),
								holderTopOffset    = $mainHolder.offset().top,
								holderBottomOffset = $mainHolder.offset().top + $mainHolder.outerHeight();
							...
							if ( enableFocus ) {
								...
								window.addEventListener(
									...
									function ( e ) {
										...
										if ( ( e.deltaY > 0 && currentPosition < holderTopOffset ) || ( e.deltaY < 0 && holderBottomOffset < currentPosition + qodefAddonsCore.windowHeight ) ) {
							...
							$holder.off().on(
								...
								function ( e ) {
									if ( options.loop ) {
										...
										//it should be prevented if going down and is not the beginning of the slider or it is going up and it is not the end of the slider
										if ( ( e.originalEvent.deltaY < 0 && ! $holder[0].swiper.isBeginning ) || ( e.originalEvent.deltaY > 0 && ! $holder[0].swiper.isEnd ) ) {
											e.preventDefault();
										}

										if ( enableFocus ) {
											if ( (e.originalEvent.deltaY < 0 && $holder[0].swiper.isBeginning) ) {
												qodefAddonsCore.qodefScroll.disable();
												$( 'html, body' ).stop().animate(
													{
														scrollTop: Math.max( 0, holderTopOffset - qodefAddonsCore.windowHeight )
													},
													1200,
													function() {
														qodefAddonsCore.qodefScroll.enable();
											...
											} else if ( e.originalEvent.deltaY > 0 && $holder[0].swiper.isEnd ) {
												qodefAddonsCore.qodefScroll.disable();
												$( 'html, body' ).stop().animate(
													{
														scrollTop: holderBottomOffset
													},
													1200,
													function() {
														qodefAddonsCore.qodefScroll.enable();
		...
		getFixedImageSize: function ( $holder, $item ) {
			...
			if ( $squareItem.length ) {
				...
				if ( squareItemImageWidth !== squareItemImageHeight ) {
					return Math.round( squareItemImageHeight );
				} else {
					return Math.round( squareItemImageWidth );
				}
			} else {
				var size    = $holder.find( '.qodef-qi-grid-masonry-sizer' ).width(),
					padding = parseInt(
						$item.css( 'paddingLeft' ),
						10
					);

				return Math.round( size - 2 * padding ); // remove item side padding to get real item size
			}
		},
		setFixedImageProportionSize: function ( $holder, $item, size ) {
			var padding         = parseInt( $item.css( 'paddingLeft' ) ),
...
(function ( $ ) {
	...
	var qodefSliderSwitch = {
		...
		initItem: function ( $currentItem ) {
			...
			let $swiperDeviceNew = new Swiper( $deviceSlider, Object.assign( deviceSliderOptions ) );
			let $swiperTextNew = new Swiper( $textSlider, Object.assign( textSliderOptions ) );
			...
			$swiperDeviceNew.on('slideChange', function(){
				...
				if (index_deviceSlide != index_textSlide){
					$swiperTextNew.slideTo(index_deviceSlide, 1000, false);
				}
			})
		}
	};
...
(function ( $ ) {
	...
	var qodefTextMarquee = {
		init: function () {
			this.holder = $( '.qodef-qi-text-marquee' );
		...
		inRange: function ( $thisMarquee ) {
			if ( qodefAddonsCore.scroll + qodefAddonsCore.windowHeight >= $thisMarquee.offset().top && qodefAddonsCore.scroll < $thisMarquee.offset().top + $thisMarquee.height() ) {
				return true;
			}
	...
	qodefAddonsCore.shortcodes.qi_addons_for_elementor_text_marquee.qodefTextMarquee = qodefTextMarquee;
...
(function ( $ ) {
	...
	$( document ).ready(
		function () {
			qodefAddonsCoreResizeIframes.init();
		}
	);
	 ...
	 */
	var qodefAddonsCoreResizeIframes = {
		init: function () {
			var $holder = $( '.qodef-blog-shortcode' );

			if ( $holder.length ) {
				qodefAddonsCoreResizeIframes.resize( $holder );
			}
		},
	...
	qodefAddonsCore.shortcodes[shortcode].qodefAddonsCoreResizeIframes = qodefAddonsCoreResizeIframes;
					...
					$items.each(
						function () {
							var $this       = $( this ),
								...
								$price      = $swapHolder.find( '.qodef-woo-product-price span' ),
							...
							if ( height < $price.height() ) {
								height = $price.height();
							}

							width = width + 2; //Rounding fix Safari

							$swapHolder.css( { 'width': width, 'height': height} );
...
(function ( $ ) {
	...
	qodefAddonsCore.shortcodes.qi_addons_for_elementor_product_slider.qodefProductListSwap = qodefProductListSwap;
