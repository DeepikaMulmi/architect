...
if ( ! class_exists( 'Emaurri_Welcome_Page' ) ) {
	class Emaurri_Welcome_Page {
		private static $instance;

		/**
		 * get the instance of Emaurri_Welcome_Page
		 *
		 * @return self
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_emaurri_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_emaurri_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_emaurri_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'emaurri_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			add_theme_page(
				esc_html__( 'About', 'emaurri' ),
				esc_html__( 'About', 'emaurri' ),
				current_user_can( 'edit_theme_options' ),
				'emaurri_welcome_page',
				array( $this, 'welcome_page_content' )
			);

			remove_submenu_page( 'themes.php', 'emaurri_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( EMAURRI_ROOT_DIR . '/screenshot.png' ) ? EMAURRI_ROOT . '/screenshot.png' : EMAURRI_ROOT . '/screenshot.jpg';

			emaurri_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_emaurri_welcome_page' === $hook ) {
				wp_enqueue_style( 'emaurri-welcome-page-style', EMAURRI_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Emaurri_Welcome_Page::get_instance();
}
